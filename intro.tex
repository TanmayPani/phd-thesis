...
measured in one process they may be used to predict any other. Requiring the
...
\begin{equation}
	...
	\frac{8\pi\alpha_s}{t}\,\hat{P}_{ba}(z)\,
	\overline{\sum}\big|\mathcal{M}_{n}\big|^2,
	\label{eq:MEcollinear}
\end{equation}
with \(\hat{P}_{ba}(z)\) the unregularized splitting kernel. The emission phase space factorizes likewise,
\begin{equation}
...
the azimuth \(\phi\),
\begin{align}
	d\sigma_{n+1}  &= d\sigma_n\,\frac{\alpha_s}{2\pi}\,\frac{dt}{t}\,\hat{P}_{ba}(z)\,dz .
...
\(C_A = 3\), and \(T_R = 1/2\).   The \emph{plus prescription},
\begin{equation}
...
terms enforce the momentum and flavor sum rules order by order. 
