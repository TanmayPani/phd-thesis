...
\emph{dependence} on \(\mu\) is not universal but perturbative. Requiring the
physical cross section of Eq.~\ref{eq:factorization} to be independent of the
arbitrary scale \(\mu\), exactly as for the coupling in
Sec.~\ref{subsec:runningCoupling},
\begin{equation}
...
Raising \(\mu\) exposes collinear splittings from which a resolved parton may
have descended. Consider the hard process of Eq.~\ref{eq:factorization}
accompanied by one additional parton, emitted at small angle in a branching
\(a\to b\,c\) off an external leg. Let \(b\) retain a longitudinal fraction
\(z\) of the parent momentum and carry transverse momentum \(\mathbf{k}_T\),
so that the internal line \(a\) acquires a virtuality
\begin{equation}
	t \equiv p_a^2 = \frac{k_T^2}{z(1-z)},
	\label{eq:collinearVirtuality}
\end{equation}
vanishing in the exactly collinear limit \(k_T\to 0\); at fixed \(z\),
\(dt/t = dk_T^2/k_T^2\). In that limit the \((n{+}1)\)-parton squared matrix
element factorizes onto the \(n\)-parton one through the near-on-shell
propagator \(1/t\),
\begin{equation}
	\overline{\sum}\big|\mathcal{M}_{n+1}\big|^2 \;\simeq\;
	\frac{2g^2}{t}\,\hat{P}_{ba}(z)\,
	\overline{\sum}\big|\mathcal{M}_{n}\big|^2,
	\label{eq:MEcollinear}
\end{equation}
with \(\hat{P}_{ba}(z)\) the unregularized splitting kernel and
\(g^2 = 4\pi\alpha_s\). The emission phase space factorizes likewise,
\begin{equation}
	d\Phi_{n+1} = d\Phi_n\,\frac{1}{16\pi^2}\,dt\,dz\,\frac{d\phi}{2\pi}.
	\label{eq:PScollinear}
\end{equation}
Inserting Eqs.~\ref{eq:MEcollinear} and~\ref{eq:PScollinear} into
\(d\sigma \propto \overline{\sum}|\mathcal{M}|^2\,d\Phi\) and integrating over
the azimuth \(\phi\),
\begin{align}
	d\sigma_{n+1} &= d\sigma_n\,\frac{2g^2}{16\pi^2}\,
	\frac{dt}{t}\,\hat{P}_{ba}(z)\,dz \notag\\
	&= d\sigma_n\,\frac{\alpha_s}{2\pi}\,\frac{dt}{t}\,\hat{P}_{ba}(z)\,dz .
	\label{eq:collinearFactorized}
\end{align}
The virtuality \(t\) plays the role of the evolution scale; identifying
\(dt/t = d\mu^2/\mu^2\), each such splitting contributes a differential
emission probability
\begin{equation}
	d\mathcal{P}_{i\leftarrow j} = \frac{\alpha_s(\mu^2)}{2\pi}\,
	\hat{P}_{ij}(z)\,dz\,\frac{d\mu^2}{\mu^2}.
	\label{eq:collinearEmission}
\end{equation}
Each additional order in \(\alpha_s\) carries a factor
\(\int dt/t = \ln(\mu^2/\mu_0^2)\). Summing these collinear logarithms, and
combining the real emissions of Eq.~\ref{eq:collinearEmission} with the
virtual, no-emission contributions that promote \(\hat{P}_{ij}\to P_{ij}\)
with the plus prescription and \(\delta(1-z)\) terms introduced below, resums
the series into the Dokshitzer-Gribov-Lipatov-Altarelli-Parisi (DGLAP)
equations \cite{dglap-gl, dglap-ap, dglap-d, Ellis_Stirling_Webber_1996},
\begin{equation}
...
probability that a parton \(j\) branches collinearly to yield a parton \(i\)
carrying a fraction \(z\) of its momentum. At leading order there are four
independent kernels, one for each allowed branching; they are collected with
their diagrams in Table~\ref{tab:splitfns}, with color factors \(C_F = 4/3\),
\(C_A = 3\), and \(T_R = 1/2\).
...
\begin{table}[htbp]
	...
	\begin{tabular}{
		...
		\toprule
		Kernel & Diagram & \(P_{ij}(z)\) \\
		\midrule
		\(P_{qq}\) & \splitvertex{fermion}{q}{}{fermion}{q}{z}{gluon}{g}{1-z} &
		\(\displaystyle C_F\left[\frac{1+z^2}{(1-z)_+} + \frac{3}{2}\delta(1-z)\right]\) \\
		\(P_{gq}\) & \splitvertex{fermion}{q}{}{gluon}{g}{z}{fermion}{q}{1-z} &
		\(\displaystyle C_F\,\frac{1+(1-z)^2}{z}\) \\
		\(P_{qg}\) & \splitvertex{gluon}{g}{}{fermion}{q}{z}{anti fermion}{\bar{q}}{1-z} &
		\(\displaystyle T_R\left[z^2 + (1-z)^2\right]\) \\
		\(P_{gg}\) & \splitvertex{gluon}{g}{}{gluon}{g}{z}{gluon}{g}{1-z} &
		\(\displaystyle 2C_A\left[\frac{z}{(1-z)_+} + \frac{1-z}{z} + z(1-z)\right]
...
\end{table}
 ...
 The \emph{plus prescription},
\begin{equation}
	\int_0^1 dz\, \frac{g(z)}{(1-z)_+} =
	\int_0^1 dz\, \frac{g(z) - g(1)}{1-z},
	\label{eq:plusPrescription}
\end{equation}
regularizes the \(z\to 1\) soft-gluon singularity, while the \(\delta(1-z)\)
terms enforce the momentum and flavor sum rules order by order. In that soft
limit the diagonal kernels reduce to
\begin{equation}
	P_{qq}(z)\xrightarrow{z\to1}\frac{2C_F}{(1-z)_+},\qquad
	P_{gg}(z)\xrightarrow{z\to1}\frac{2C_A}{(1-z)_+},
	\label{eq:splitSoft}
\end{equation}
so a radiating gluon is more singular than a radiating quark by the ratio
\(C_A/C_F = 9/4\), already fixing the differing activity of quark- and
gluon-initiated cascades exploited in later chapters.
