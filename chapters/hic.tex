...
\end{figure}
Anisotropic flow is the azimuthal anisotropy in the momentum distribution of the final state particles. 
%
The mean free path of individual particles in the QGP medium ($l$) is much smaller than system size ($R$). Hence, the initial state coordinate space asymmetry in non-central heavy ion collisions generates a pressure gradient in the direction of the impact parameter $b$. 
%
This is schematically shown in Fig. \ref{fig:anisotropycartoon}. 
%
This spatial asymmetry can then translate into a momentum anisotropy as the system expands, meaning that particles are more likely to be emitted along the plane defined by the beam axis and the impact parameter of the collision (called the reaction plane, \(\Psi_{\rm RP}\)), where the pressure gradients are the highest.  
%
This can be characterized by the Fourier decomposition of the azimuthal particle distribution with respect to \(\Psi_{\rm RP}\) ~\cite{Bielcikova:2003ku,Voloshin:1994mz}:
\begin{equation}
	\frac{dN}{d(\phi - \Psi_{RP})} = \frac{N_0}{2\pi} \left( 1 + 2 \sum_{n=1}^{\infty} v_n \cos[n(\phi - \Psi_{RP})] \right),
	\label{fourier_azi_part_distr}
\end{equation}
The flow coefficients $v_{n}$, the $n^{\rm th}$ order term in the Fourier decomposition of the final state particle azimuthal distribution is given by,
\begin{equation}
	\label{flow}
	v_{n} = \left< \cos([n(\phi - \Psi_{\rm RP})]) \right>
\end{equation}
Due to the symmetry of the collision geometry, the most prominent contribution is from the second order coefficient $v_2$, called elliptic flow.

\subsubsection{Event plane reconstruction}
The reaction plane, \(\Psi_{\rm RP}\) cannot be directly measured and is approximated by the second-order symmetry plane, $\Psi_{2,EP}$, referred to as the ``event plane"  in this text, for simplicity.  
%
...
%
Within the overlap region, symmetry planes are generated from initial asymmetries in the nucleon distributions and can be quantified by a harmonic decomposition ~\cite{Poskanzer:1998yz}.   
%
...
\end{equation}
%
the weighted vector \(\mathbf{Q}_n = Q_{x, n}\hat{\mathbf{x}}+ Q_{y, n}\hat{\mathbf{y}}\) are given by~\cite{Poskanzer:1998yz}:
%
\begin{align}
...
%
The event plane is calculated event-by-event, following a procedure similar to Ref.~\cite{CASTILHO2018369} using charged tracks with $0.2 < p_{\rm  T, track} < 1.0$ \GeVc measured within the TPC.  
%
The approach is called the Modified Reaction Plane (MRP) method \cite{Agakishiev:2014ada,CASTILHO2018369,Adams_2005}.
% 0.2-2.0
...
%
An upper limit of 1.0 GeV/c is used in the calculation of the event plane to exclude the momentum range of particles used in correlation functions from the calculation of the event plane which is used to characterize the near-side jets. 
%
...
%
It is done by calculating a modified $\mathbf{Q}_2$-vector, obtained by subtracting an event averaged $\langle\mathbf{Q}_2 \rangle$ from each event's nominal $Q$-vector and done for 10\% centrality intervals and 4 cm $v_z$ intervals. 
%

The recentering approach, which drastically improves the uniformity of the event plane, is however, unable to remove the higher harmonics of $\Psi_{n,EP}$ \cite{Poskanzer:1998yz}.  
%
They are removed through a second correction step, referred to as  the shifting method, applied event-by-event  to make the event-plane angle isotropic in the lab frame \cite{Voloshin:2008dg}. 
%
...
\end{equation}
%
where the brackets denote an average over events.  
%
We require the vanishing of each n-th Fourier moment up to 20th order.  
%
Similarly to recentering, the shifting correction is done separately for 10\% centrality intervals and 4-cm $v_z$ intervals. 
%
Additional details of the recentering and shifting corrections can be found in Refs.~\cite{Wang:2012bga,Barrette:1997pt,Poskanzer:1998yz}
 ...
 %
 Because an ideal event-plane resolution is equal to 1, for non-ideal cases, the value of the coefficients will be raised by applying the correction. 
 %
 Thus, $R_n$ impacts the flow-modulated background for these correlations, as described in Sect.~\ref{sect:CorrBkgd},
...
Since QGP is extremely short-lived, studying it using external probes is not practical. 
%
Internal probes of QGP can either be \emph{bulk probes} or \emph{hard probes}
%
\subsection{Bulk probes}
\begin{figure}[h]
	\centering
...
\end{figure}
$v_2$ was measured by the STAR and PHENIX experiments at RHIC for several particle species and is shown in Fig. \ref{fig:AnisotropyResults}. Large positive $v_2$ values measured by both the experiments indicated that the medium that exists in heavy ion collisions behaves as a strongly-coupled liquid. A scaling of the $v_2$ values with the number of constituent quarks (2 for mesons, 3 for baryons) yielded a universal curve as a function of $p_{\rm T}/n_q$ suggesting the flow dynamics were developed in a medium with quark and gluon degrees of freedom, i.e. QGP.

\subsection{Hard probes}
The main evidence of jet interaction with the QGP is through \textit{jet quenching}, where the jets lose energy through their interactions with the medium particles. The two main mechanisms for this process are collisional and radiative energy losses. Collisional energy loss happens due to the elastic scattering between the parton and the medium constituents and usually dominates for lower energy partons. Radiative energy loss happens due to induced gluon radiation (\textit{bremsstrahlung}) through inelastic scatterings with the medium particles \cite{MAJUMDER201141,QinWang2024,Burkeetal}. For partons with \ptGeq{}{5} , this is usually the dominant source of energy loss \cite{LandoltBornstein2010:sm_lbs_978-3-642-01539-7_16}. One of the first measurements of jet quenching was performed by the STAR experiment\cite{PhysRevLett.91.072304}, well before it became feasible to reliably reconstruct a sufficient number of jets in heavy-ion collisions. In this measurement shown in Fig. \ref{fig:angularcorr}, the azimuthal angular correlation ($\Delta\phi$) of all charged hadrons reconstructed by the STAR detector with the leading $p_\mathrm{T}$ hadron (used as a proxy for the jet) was compared between p + p, Au + Au, and d + Au collisions. The p + p and d + Au both show peaks around $\Delta\phi\approx 0$ and $\Delta\phi\approx \pi$ as expected, given the back-to-back nature of the hard scattered parton. However, for Au + Au collisions, the away side peak ($\Delta\phi\approx \pi$) completely vanishes. This was the expectation because depending on the distance from the center of the collision where the initial back-to-back partons were produced, one of the partons would travel a larger distance in the QGP medium and therefore lose more energy than the other one.
...
A collection of measurements of nuclear modification factors (Eq. \ref{RAA}) for identified particles from the PHENIX experiments, shown in Fig. \ref{fig:singleparticleraa} allowed for a more nuanced look at jet quenching. For example, direct photons ($\gamma$) do not interact strongly, and hence have no yield modification in the medium. All strongly interacting particles show varying levels of suppression from 0.2 - 0.6, again confirming their energy loss in the presence of QGP.
...
Although measurements with high $p_\mathrm{T}$ hadrons help us understand jet quenching mechanisms in the medium, they suffer from survival bias, as these hadrons arise from a subsample of jets which usually undergo the least amount of quenching \cite{PhysRevC.99.051901}. Fully reconstructed jets, however, are mostly unbiased probes of energy flow at high $p_\mathrm{T}$, and offer a wealth of substructure observables which allow us to look at energy flow within the jet more differentially.

The STAR experiment measured the modification of the charged particle jets\footnote{Charged particle jets are formed with clustered charged tracks only and contain no calorimeter hits (towers) as input.} $p_\mathrm{T}$ spectra using anti-$k_{\rm T}$ jets between central and peripheral Au + Au collisions at \sNN{200}{GeV} \cite{STARChargedJet}. This is shown in Fig. \ref{fig:chargedparticlercp} using the nuclear modification factor ($R_{CP}$) defined as
\begin{equation}
	R_{CP}(p_\mathrm{T}, \eta, b) = \frac{d^2N/d\eta dp_\mathrm{T}}{\left< N_{\rm bin}(b) \right>} \biggm\lvert_{\rm cent} \times \frac{\left< N_{\rm bin}(b) \right>}{d^2N/d\eta dp_\mathrm{T}} \biggm\lvert_{\rm peri}
\end{equation}
and compared to the measured $R_{CP}$ for charged hadrons from STAR at the same energy, and to the measured $R_{CP}$ for charged hadrons and jets from the ALICE and ATLAS experiments at \sNN{2.76}{TeV}. First, the charged hadrons $R_{CP}$ is found to be the same over the same range of $p_\mathrm{T}$ at RHIC and LHC energies (which are an order of magnitude apart). The charged jet $R_{CP}$ is also found to have similar values between RHIC and LHC energies with roughly 60\% of the energy lost outside the jet cone in central collisions with respect to peripheral collisions, with no significant $p_\mathrm{T}$ dependence. This lack of $p_\mathrm{T}$ dependence for charged jets is in contrast to the $p_\mathrm{T}$ dependence observed for the charged hadrons $R_{CP}$. This may be because the inclusive hadron distribution usually arises from the leading hadron in the corresponding jet, and is therefore is sensitive to the fragmentation function. This fragmentation process can therefore introduce the $p_\mathrm{T}$ dependence of $R_{CP}$ in hadrons. 
\begin{figure}
	\centering
