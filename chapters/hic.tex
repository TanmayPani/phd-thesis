...
%
This algorithm becomes the \emph{\(k_\mathrm{T}\) algorithm} for \(p=1\), \emph{Cambridge-Aachen} for \(p=0\) and the most widely used \emph{anti-\(k_\mathrm{T}\) algorithm} for \(p=-1\). 
%
\begin{figure}
	\centering
...
%
The \emph{FastJet} library efficiently implements these algorithms and a few more, and is the de-facto standard in the High Energy Physics (HEP) community. 
%
It also provides various recombination schemes. 
\begin{itemize}
	\item \emph{E scheme}, just adds the four momenta of the jet candidates.
	\item \emph{$p_t$, $p_t^2$, $E_t$ and $E_t^2$ schemes} involve a preprocessing stage rescaling the energy to be equal to the 3-momentum for the
...
\end{itemize}
Out of these, E-scheme is the default in FastJet and the most commonly used.
...
and impact parameter \(b\) from Glauber model is mapped with the experimental data through a particle production model called the Two-Component Model\CITETHIS. 
%
