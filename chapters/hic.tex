...
%
For example, in a clean reaction like \(e^+e^-\to q\bar{q}\). If one of the quark emits a gluon, then depending on the emission angle of the gluon, we might cluster 2 or 3 jets in the final state. We will discuss more about jet finding in \ref{sec:jetAlgo}
...
\end{equation}
Here \(p_\mathrm{T, \{1,2\}}\) are the momenta of the two
branches that had been merged together and \(\Delta R_{12}\) is their geometric distance in the \etaphi plane. When the softer branch fails the
criterion it is removed from the jet and the procedure continues until it is satisfied. The remaining particles in the jet constitute
the soft drop groomed jet. A convenient choice can be made for the soft threshold parameter \(z_\mathrm{cut}\) and the angular exponent \(\beta\). For \(\beta = 0\) the soft drop procedure reduces to the modified mass drop tagger (mMDT)
...
\section{Spacetime picture of QGP}
...
The heavy ion collision and the evolution of the fireball, as depicted in Figure~\CITETHIS, has several stages, and the measurement of the final state particles can be affected by one or all of these stages depending on the production mechanism and interaction time within the medium.  
%
